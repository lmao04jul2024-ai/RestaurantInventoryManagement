/**
 * Central API client — mirror of packages/web/src/lib/api.ts for React Native.
 * - Attaches Authorization + X-Tenant-ID headers automatically.
 * - On 401, performs a single-flight token refresh then retries once.
 *
 * Difference vs web: on unrecoverable refresh failure we only clearAuth();
 * the RootNavigator auth-gate switches back to the Login stack automatically
 * (there is no window.location to redirect here).
 */
...
async function refreshAccessToken(): Promise<string> {
  ...
  try {
    ...
    // Refresh failed — session is dead; the auth gate handles navigation
