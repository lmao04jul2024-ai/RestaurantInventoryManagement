...
## Context
- Tracker 130/144 (Weeks 1–20 done; 14 skipped per user; 148 delivered). Phase 5 continues.
...
- [ ] **21.2** Staff assignment UI (assign/unassign buttons, staff dropdown from /api/staff, show assigned staff on card)
- [ ] **21.3** Prep time display (prepStartedAt → elapsed, readyAt when ready, color by target)
- [ ] **21.4** Prep time target config in tenant settings (MANAGER+ editable, default 15)
- [ ] **21.5** Capacity indicator (active count vs capacity, warning when near limit)
- [ ] **21.6** SSE field bindings (assignedStaff, prepElapsed, prepTargetMet re-render kitchen board)

### Docs
- [ ] Update OpenAPI (new kitchen endpoints, status enum docs, staff assignment, prep-time fields)
- [ ] Update admin guide (kitchen workflows, staff assignment, prep time targets, capacity)

## Status
- Week 20 complete: all 6 deliverables + tests + docs committed. 275 API tests / 133 web tests green.
...
- [ ] openapi.yaml + admin guide updated; tracker 21.1–21.6 (120/144); commits api → web → docs/bookkeeping; memory push
