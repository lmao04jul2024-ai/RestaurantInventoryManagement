...
- **Diagnosis order that worked:** (1) grep the reported capability (`currentPassword|change password`) → absent; (2) drive the real UI to see the actual affordance list; (3) probe the API surface with curl (404 vs 401 distinguishes "missing route" from "guarded route"); (4) only then design.
- **Design constraint that shaped the fix:** `resolveTenant` rejects `PLATFORM_ADMIN` on every tenant surface with `403 PLATFORM_ADMIN_FORBIDDEN`, so a repair hung off a tenant route would be unusable for the operator. The new `PATCH /api/auth/password` is **authenticate-only** and scopes the write to `req.user.userId`; it deliberately mirrors the L052 rule that credential-entry/self-service auth routes must not be tenant-gated. Requires the current password (a lifted token must not rotate a credential), rejects reuse, revokes **all** refresh sessions, and writes a fail-open `security:auth.password_changed` audit row.
- **Rule:** When a user says "fix X", first prove X exists *and* how it is reached. If the affordance is absent, the deliverable is a new capability (endpoint + UI + tests + docs), and the UI must land where the user actually reported the problem (here: `components/layout/user-menu.tsx`, shared by the `(platform)` console shell and the `(dashboard)` shell — not only one of them).
- **Live-verification hygiene:** changing a real credential to prove it works means you must **restore it** (rotated to `Rotated!Platform2026`, then back to the seeded `ChangeMe!Platform2026`, final login verified 200 as PLATFORM_ADMIN). Also: `RATE_LIMIT_AUTH_MAX=10 per 60s` throttles scripted login/change loops — a `429 RATE_LIMITED` mid-script is the limiter, not a bug; re-run after the window instead of "fixing" the code.
