...
- [x] **S3.4** Operator attention list: admin console banner/queue of tenants needing manual action (trial ending, PAST_DUE) *(GET /api/platform/attention — server-derived reasons SUSPENDED/PAST_DUE/CANCELLED/TRIAL_ENDING (within 7d of 30d trial from createdAt)/TRIAL_EXPIRED; PLATFORM_ADMIN-gated; 4 supertest tests; web `platform-attention-queue.tsx` card on the console workspace list replacing the page-local banner, usePlatformAttention hook shares the platform-tenants key prefix so operator changes refresh the queue; openapi documented)*
